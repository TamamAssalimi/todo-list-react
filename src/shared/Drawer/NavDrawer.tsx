...
export default function NavDrawer() {
  ...
  const sideList = (side: DrawerSide) => (
    <div style={{ width: 250 }} role="presentation" onClick={toggleDrawer(side, false)} onKeyDown={toggleDrawer(side, false)} >
      <SuspenseContainer>
        <Greet name={name} />
      </SuspenseContainer>
      <List>
        {RouteConfig.filter((item: IRouteConfig) => item.visible).map(
          (item: IRouteConfig, index) => {
            ...
            return (
              <ListItem onClick={() => history.push(item.goto)} button key={index} style={{ ...style }}>
